...
class ReportDatabase:
    ...
    def _generate_and_save_image_spark(self, title: str, summary: str, area: str) -> str:
        ...
        try:
            ...
            if image_base64:
                # 生成文件名
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                filename = f"spark_report_{timestamp}.png"
                filepath = os.path.join(self.img_path, filename)
    ...
    def _generate_and_save_image(self, title: str, summary: str, area: str) -> str:
        """
        【已弃用】旧的图片生成方法，保留以兼容
        新版本请使用 _generate_and_save_image_spark
        """
        return self._generate_and_save_image_spark(title, summary, area)

...
def join_housing_data(area_name: str, limit: int = 100) -> str:
    """
    直接拼接房源数据的简单版本
    """
    data_list, headers = query_house_data_by_area(area_name, limit)
    ...
    for i, house in enumerate(data_list, 1):
        output_lines.append(f"\n{i}. ", end="")
        # 只显示关键信息
        key_info = []
        for field in ['community', 'price', 'area', 'room_layout']:
            if field in house and house[field]:
                key_info.append(str(house[field]))
        output_lines.append(" | ".join(key_info))
...
if __name__ == "__main__":
    a=get_area_statistics("丰台")
    print(a['year_distribution'])
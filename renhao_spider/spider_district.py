...
class SpiderDistrict:
    ...
    def build_url(self, district=None, page=1):
        """构建URL
        ...
        Returns:
            str: 完整URL
        """
        if district and district in BEIJING_DISTRICTS:
            district_pinyin = BEIJING_DISTRICTS[district]
            url = f"{self.base_url}{district_pinyin}/ershoufang/"
        else:
    ...
    def crawl(self, district=None, max_pages=None):
        """爬取房源数据
        ...
        Returns:
            list: 爬取到的房源列表
        """
        location_desc = district if district else "全城"
        ...
        while True:
            ...
            url = self.build_url(district=district, page=page)
    ...
    def save_to_csv(self, filename=None):
        ...
        fieldnames = ['crawl_district', 'title', 'price', 'price_unit', 
...
def interactive_menu():
    """交互式菜单"""
    spider = SpiderDistrict()
    districts = list(BEIJING_DISTRICTS.keys())
    
    while True:
        print("\n" + "=" * 60)
        print("  58同城北京二手房爬虫（按区域）")
        print("=" * 60)
        print("\n请选择操作:")
